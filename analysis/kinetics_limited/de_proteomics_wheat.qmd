...
Everything is expressed as fold-change relative to the time-matched control, so $r(t) = 2^{\text{logFC}_{\text{RNA}}(t)}$ and $p(t) = 2^{\text{logFC}_{\text{protein}}(t)}$, both starting at 1.

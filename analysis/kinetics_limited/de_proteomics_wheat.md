...
control, so $r(t) = 2^{\text{logFC\_RNA}(t)}$ and
$p(t) = 2^{\text{logFC\_protein}(t)}$, both starting at 1.
...
**Two structural properties worth documenting:** \* `regulated` requires
`rna_resp`. Amplitude is meaningless without an RNA signal to scale —
for a flat transcript, $a \cdot 0 = 0$ for any $a$. This gate is what
makes `protein_only` a separate category rather than a spuriously
extreme amplitude. \* `concordant` vs `kinetic_lag` is a labelling
split, not a test. Both mean “the kinetic null was sufficient.” The slow
cutoff just separates fast-turnover proteins from slow ones for
readability; genes near the line are on a continuum.
